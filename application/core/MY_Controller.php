...
class MY_Controller extends MX_Controller
{
	...
	public function __construct()
	{
		...
		//HÄR ÄR ANVÄNDAREN INLOGGAD HÄMTA ALL DATA OM ANVÄNDAREN TILL $this->u
		//$this->u = $this->ion_auth->user()->row();
		...
		{
			$this->layout = FALSE;
			$this->view = 'ajax_message';
		}

		//SET THE MODULE NAME IF ANY
		$this->_module = CI::$APP->router->fetch_module();
		$this->u               = $this->ion_auth->user()->row();
		$this->u->groups       = $this->ion_auth->get_users_groups($this->u->id)->result();
	...
	{
		//$this->data = escapeHtml($this->data);
		...
		if ($this->view !== FALSE)
		{
			...
			//$modules_locations = key($this->config->item('modules_locations'));

			// If $this->view isn't empty, load it. If it isn't, try and guess based on the controller and action name
			$view = (!empty($this->view)) ? $this->view : $this->router->class . '/views/' . $this->router->method;
			//Dose the view file exist?


			$view = '../modules/' . $view.'.php';
